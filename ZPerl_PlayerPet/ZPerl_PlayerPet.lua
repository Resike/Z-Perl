...
function XPerl_Player_Pet_OnLoad(self)
	...
	for i, event in pairs(events) do
		if string.find(event, "^UNIT_") or string.find(event, "^INCOMING") then
			...
			elseif IsClassicOrBCC and event == "UNIT_HAPPINESS" and classFileName == "HUNTER" then
				if pcall(self.RegisterUnitEvent, self, event, "pet") then
					self:RegisterUnitEvent(event, "pet")
				end
			else
	...
	if XPerl_ArcaneBar_RegisterFrame then
		XPerl_ArcaneBar_RegisterFrame(self.nameFrame, (not IsClassicOrBCC and UnitHasVehicleUI("player")) and "player" or "pet")
	end
...
local function XPerl_Player_Pet_Update_Control(self)
	if UnitIsCharmed(self.partyid) and UnitIsPlayer(self.partyid) and (not IsClassicOrBCC and not UnitInVehicle("player") or true) then
...
function XPerl_Player_Pet_Events:PLAYER_ENTERING_WORLD()
	if not IsClassicOrBCC and UnitHasVehicleUI("player") then
		self.partyid = "player"
